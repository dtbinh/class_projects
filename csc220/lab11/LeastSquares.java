...
public class LeastSquares implements DrawListener {
	...
	public void mousePressed(double x0, double y0) {
		...
		/**comment*/
		// update sumx, sumx2, sumy, sumy2 values here
		sumx += x[N];
		sumy += y[N];
		sumx2 += x[N]*x[N];
		sumy2 += y[N]*y[N];
		...
		// declare double values a and b, calculate a and b values
		double a = (N*sumxy-sumx*sumy) / (N*sumx2 - sumx*sumx);
		double b = (sumy-a*sumx)/N;
		
		// based on sumx, sumx2, sumy, sumy2
		draw.line(0, b, 1, a + b);
		
		// draw the points and vertical lines
		for (int i = 0; i < N; i++) {
			draw.setPenColor(Color.BLUE);
			draw.filledCircle(x[i], y[i], 0.005);
			draw.setPenColor(Color.GRAY);
			// draw the vertical line from the point to regression
			// please use method draw.line(xi, yi, xi, y)
			// where (xi, yi) is the point you clicked, (xi, y) is the point on line
